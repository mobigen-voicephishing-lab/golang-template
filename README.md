...
│   │   ├── inbound/
│   │   │   └── http/
│   │   │       ├── dto/             # 요청/응답 구조체 (에러 코드는 domain 패키지)
...
│   ├── domain/
│   │   ├── sample.go                # 도메인 엔티티
│   │   └── errors.go                # AppError, 에러 코드/메시지 상수
│   ├── usecase/
│   │   └── sample/
...
│   ├── infrastructure/
│   │   ├── config/                  # 환경변수, 설정 파일, 빌드 정보 (configuration.go)
│   │   ├── logger/                  # Logger 인터페이스 + LogrusLogger 구현체
│   │   └── db/                      # GORM 연결, 마이그레이션
│   ├── bootstrap/
...
type SampleHandler struct {
    Log     logger.Logger  // 인터페이스 주입 (LogrusLogger, noopLogger 등)
...
> `wire.go` 내 주석 처리된 `NewSampleHandlerWithUsecase` 코드를 참고하세요.

### 3.9. 에러 처리

도메인 레이어에서 `AppError`를 생성하고 핸들러까지 전파합니다.
`AppError`는 도메인 에러 코드(`Code`)만 갖고 있으며, HTTP 상태 코드는 어댑터 레이어(`httpStatusForAppError`)에서 코드를 보고 결정합니다.

```go
// domain/errors.go — HTTP 상태 코드 없이 도메인 코드만 사용
func NewNotFoundError(msg string) AppError {
    return AppError{Code: ErrNotFound, Message: msg}
}

// 핸들러에서 처리 — FailApp 내부에서 httpStatusForAppError로 HTTP 상태 코드 결정
if ae, ok := err.(domain.AppError); ok {
    return FailApp(c, ae)
}
```
...
// noopLogger 테스트용 Logger 인터페이스 no-op 구현체
...
func TestSampleHandlerGetByID(t *testing.T) {
    ...
    h := handler.NewSampleHandlerWithUsecase(noopLogger{}, mockUsecase)
